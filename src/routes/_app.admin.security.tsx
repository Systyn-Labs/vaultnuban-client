...
import { adminMfaStatusQuery } from "@/data/queries";
...
export const Route = createFileRoute("/_app/admin/security")({
  head: () => ({ meta: [{ title: "Admin security · VaultNUBAN" }] }),
  component: () => (
...
function AdminSecuritySettingsPage() {
  const { data: status } = useSuspenseQuery(adminMfaStatusQuery);
...
function EnrolledPanel({ recoveryCodesRemaining }: { recoveryCodesRemaining: number }) {
  ...
  const regenerate = useMutation({
    mutationFn: () =>
      adminHttp().post<{ recovery_codes: string[] }>("/internal/mfa/recovery-codes/regenerate", {
        code,
      }),
    onSuccess: (r) => {
  ...
  return (
    <div className="max-w-lg space-y-4 border bg-surface p-5">
      ...
      <p className="text-[12px] text-muted-foreground">
        This admin account requires a fresh authenticator code (or a recovery code) before any
        action that mutates a tenant or platform record.
      </p>
...
function EnrollmentPanel() {
  ...
  const beginSetup = useMutation({
    mutationFn: () => adminHttp().post<MfaSetupResponse>("/internal/mfa/setup"),
  ...
  const confirm = useMutation({
    mutationFn: () => adminHttp().post<{ enabled: boolean }>("/internal/mfa/enable", { code }),
    onSuccess: () => {
  ...
  if (!setup) {
    return (
      <div className="max-w-lg space-y-4 border bg-surface p-5">
        ...
        <p className="text-[12px] text-muted-foreground">
          This is required before you can suspend a tenant, revoke an API key, reset a user's MFA,
          or perform other sensitive platform actions.
        </p>
        <Button
          className="gap-1.5"
...
function RecoveryCodesDisplay({ codes, onDone }: { codes: string[]; onDone: () => void }) {
  function download() {
    ...
    a.href = url;
    a.download = "vaultnuban-admin-recovery-codes.txt";
    a.click();
